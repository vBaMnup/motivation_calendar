...
from pictures.string_split import (get_text_from_json, make_strings_from_text,
                                   get_key_by_dict)
...
from settings.config import (BACKGROUNDS_DIR, CALENDAR_DIR,
                             ...
                             COORD_PHRASE_RIGHT, FONTS_DIR, QUOTES_FILE_DIR)
...
class CreateImage:
    def __init__(self):
        ...
        ) if MONTH in WINTER else choice(
            list(Path(BACKGROUNDS_DIR).glob('*.*')))
    ...
    def create_temp_image(self, text_lines: List[str] = None,
                          zodiac: str = None) -> Image:
        text_color = (255, 255, 255)
        outline_color = (0, 0, 0)
        outline_width = 20
        if zodiac:
            zodiac = get_key_by_dict(ZODIAC_SINGS, zodiac.title())
            zodiac_img = Image.open(
                f'{HOROSCOPE_IMG_DIR}{zodiac}.png').convert('RGBA')
            ...
            font = ImageFont.truetype(f"{FONTS_DIR}you20133.ttf",
                                      size=font_size)
            ...
            for text in text_lines:
                ...
                draw.text((width / 2 - 2, y_offset + text_height / 2),
                          text=text,
                          fill=(0, 0, 0),
                          font=font,
                          anchor="ma")
                draw.text((width / 2, y_offset + text_height / 2),
                          text=text,
                          fill=text_color,
                          font=font,
                          anchor="ma")
    ...
    def make_wallpaper_v2(self, tg_id, input_zodiac=None):
        ...
        if input_zodiac:
            ...
            if position == 'center':
                ...
                self.calendar.paste(calendar_img, coords['calendar'],
                                    calendar_img)
                self.calendar.paste(zodiac_img, coords['zodiac'], zodiac_img)
            elif position == 'right':
                self.calendar.paste(phrase_img, coords_right,
                                    phrase_img)
                self.calendar.paste(calendar_img, coords['calendar_with_horo'],
                                    calendar_img)
                self.calendar.paste(zodiac_img, coords['zodiac'], zodiac_img)
            else:
                self.calendar.paste(phrase_img, coords_left,
                                    phrase_img)
                self.calendar.paste(calendar_img, coords['calendar_with_horo'],
                                    calendar_img)
                self.calendar.paste(zodiac_img, coords['zodiac'], zodiac_img)
    ...
    def make_wallpaper(self, tg_id, input_zodiac=None):
        ...
        if input_zodiac:
            ...
            if 'left' in self.random_background.name:
                self.calendar.paste(phrase_img, COORD_PHRASE_LEFT,
                phrase_img)
                self.calendar.paste(calendar_img, COORD_CALENDAR_CENTER,
                                    calendar_img)
            elif 'right' in self.random_background.name:
                self.calendar.paste(phrase_img, COORD_PHRASE_RIGHT,
                phrase_img)
...
if __name__ == '__main__':
    ...
    # text_list = ['Ох, уж, эти Водолеи. Весь месяц в кураже и',
